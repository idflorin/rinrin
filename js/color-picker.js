/**
 * @file
 * Provides UI/UX progressive enhancements on Rinrin's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.rinrin.colorSchemes;
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-rinrin-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-rinrin-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes Rinrin theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-rinrin-custom-color',
      textInput.getAttribute('data-rinrin-custom-color'),
    );
  ...
  /**
   * Rinrin Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.rinrinColorPicker = {
    attach: () => {
      ...
      const colorTextInputs = once(
        'rinrin-color-picker',
        '[data-drupal-selector="rinrin-color-picker"] input[type="text"]',
      );
